    ...
    <div class="ereaders-blog-grid-text">
        ...
        <div class="ereaders-resource-image mb-3">
            ...
            <ul class="ereaders-thumb-option flex">
                <li class="text-xxs">Author: 
                    @php $author = $resource->author(); @endphp
                    @if($author && $author->is_lead && has_profile($author->username) )
                        <a class="text-xxs" href="{{ route('account.profile', $author->username ) }}">{{  $author->fullname  }} | </a>
                    @elseif($author)
                        <span class="text-xxs" >{{  $author->fullname  }} |</span>
                    @else
                        <span class="text-xxs" >Unknown Author |</span>
                    @endif
                </li>
        ...
        <div class="resource-actions">
            ...
            @if(auth()->user() && $resource->user_id == auth()->user()->id)
                ...
                <a href="{{ route('resources.delete', $resource->slug ) }}" class="btn btn-danger ereaders-readmore-btn" onclick="return confirm('Are you sure you want to delete this resource? This action cannot be undone.');"><i class="fa fa-trash text-white"></i></a>
