...
{       
    ...
    /**
     * Display pending resources for review
     *
     * @return \Illuminate\Http\Response
     */
    public function pending()
    {
        $pendingResources = Resource::withoutGlobalScopes()
            ->where('approval_status', 'pending')
            ->orderBy('submitted_at', 'desc')
            ->get();
            
        return view('admin.resources.pending', compact('pendingResources'));
    }
    ...
    {
        try {
            $resource = Resource::withoutGlobalScopes()->findOrFail($id);
            ...
            try {
                $user = $resource->user;
                if ($user) {
                    Mail::to($user->email)->send(new ResourceApprovalNotification($resource, 'approved'));
                }
            } catch (\Exception $e) {
                \Log::error('Failed to send approval email: ' . $e->getMessage());
            }

            notify()->success('Resource approved and published successfully!');
            return redirect()->back();

        } catch (\Exception $e) {
            \Log::error('Resource approval failed: ' . $e->getMessage());
            notify()->error('Failed to approve resource: ' . $e->getMessage());
            return redirect()->back();
        }
    }
    ...
    public function reject(Request $request, $id)
    {
        try {
            ...
            try {
                $user = $resource->user;
                if ($user) {
                    Mail::to($user->email)->send(new ResourceApprovalNotification($resource, 'rejected', $request->admin_notes));
                }
            } catch (\Exception $e) {
                \Log::error('Failed to send rejection email: ' . $e->getMessage());
            }

            notify()->success('Resource rejected. User has been notified.');
            return redirect()->back();

        } catch (\Exception $e) {
            \Log::error('Resource rejection failed: ' . $e->getMessage());
            notify()->error('Failed to reject resource: ' . $e->getMessage());
            return redirect()->back();
        }
    }
